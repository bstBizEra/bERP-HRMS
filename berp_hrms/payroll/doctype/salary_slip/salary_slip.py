...
import berp_hrms
from berp_hrms.hr.utils import validate_active_employee
from berp_hrms.payroll.doctype.additional_salary.additional_salary import get_additional_salaries
from berp_hrms.payroll.doctype.employee_benefit_ledger.employee_benefit_ledger import (
	create_employee_benefit_ledger_entry,
	delete_employee_benefit_ledger_entry,
)
from berp_hrms.payroll.doctype.income_tax_slab.income_tax_slab import calculate_tax_by_tax_slab
from berp_hrms.payroll.doctype.payroll_entry.payroll_entry import get_salary_withholdings, get_start_end_dates
from berp_hrms.payroll.doctype.payroll_period.payroll_period import (
	get_payroll_period,
	get_period_factor,
)
from berp_hrms.payroll.doctype.salary_slip.salary_slip_loan_utils import (
	cancel_loan_repayment_entry,
	make_loan_repayment_entry,
	process_loan_interest_accrual_and_demand,
	set_loan_repayment,
)
from berp_hrms.payroll.utils import (
...
from berp_hrms.utils.holiday_list import get_holiday_list_ranges_for_employee, get_holidays_in_ranges_map
...
class SalarySlip(TransactionBase):
	...
	if TYPE_CHECKING:
		...
		from berp_hrms.payroll.doctype.employee_benefit_detail.employee_benefit_detail import (
			EmployeeBenefitDetail,
		)
		from berp_hrms.payroll.doctype.salary_detail.salary_detail import SalaryDetail
		from berp_hrms.payroll.doctype.salary_slip_leave.salary_slip_leave import SalarySlipLeave
		from berp_hrms.payroll.doctype.salary_slip_timesheet.salary_slip_timesheet import SalarySlipTimesheet
	...
	def publish_update(self):
		...
		frappe.publish_realtime(
			event="berp_hrms:update_salary_slips",
	...
	def get_emp_and_working_day_details(self) -> None:
		...
		if self.employee:
			...
			if struct:
				from berp_hrms.payroll.doctype.salary_structure.salary_structure import make_salary_slip
	...
	def add_leave_balances(self):
		...
		if frappe.db.get_single_value("Payroll Settings", "show_leave_balances_in_salary_slip"):
			from berp_hrms.hr.doctype.leave_application.leave_application import get_leave_details
...
def enqueue_email_salary_slips(names: list | str) -> None:
	...
	frappe.enqueue("berp_hrms.payroll.doctype.salary_slip.salary_slip.email_salary_slips", names=names)
	frappe.msgprint(
