...
from berp_hrms.overrides.company import delete_company_fixtures
...
def get_regional_custom_fields():
	...
	for country in frappe.get_all("Company", pluck="country", distinct=True):
		try:
			getter = frappe.get_attr(f"berp_hrms.regional.{frappe.scrub(country)}.setup.get_custom_fields")
...
def setup_notifications():
	base_path = frappe.get_app_path("berp_hrms", "hr", "doctype")
...
def run_post_install_patches():
	...
	try:
		for patch in POST_INSTALL_PATCHES:
			...
			frappe.get_attr(f"berp_hrms.patches.post_install.{patch_name}.execute")()
	finally:
