...
frappe.ui.form.on("Expense Claim", {
	...
	make_payment_entry: function (frm) {
		return frappe.call({
			method: "berp_hrms.overrides.employee_payment_entry.get_payment_entry_for_employee",
			args: {
				dt: frm.doc.doctype,
				dn: frm.doc.name,
			},
	...
	company: function (frm) {
		...
		for (var i = 0; i < expenses.length; i++) {
			...
			frappe.call({
				method: "berp_hrms.hr.doctype.expense_claim.expense_claim.get_expense_claim_account_and_cost_center",
				args: {
	...
	set_payable_account: async function (frm) {
		...
		const account = (
			await frappe.call({
				method: "berp_hrms.hr.doctype.expense_claim.expense_claim.get_default_payable_account",
	...
	get_advances: function (frm) {
		if (frm.doc.employee) {
			return frappe.call({
				method: "berp_hrms.hr.doctype.expense_claim.expense_claim.get_advances",
				args: {
