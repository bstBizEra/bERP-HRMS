...
frappe.ui.form.on("Shift Assignment Tool", {
	setup(frm) {
		berp_hrms.setup_employee_filter_group(frm);
	},
	...
	refresh(frm) {
		...
		berp_hrms.handle_realtime_bulk_action_notification(
			frm,
	...
	render_employees_datatable(frm, employees) {
		...
		berp_hrms.render_employees_datatable(frm, columns, employees, no_data_message);
	},
	...
	bulk_assign(frm, employees) {
		...
		berp_hrms.validate_mandatory_fields(frm, selected_employees);
		frappe.confirm(
	...
	process_shift_requests(frm, status) {
		...
		berp_hrms.validate_mandatory_fields(frm, selected_requests, "Shift Requests");
		frappe.confirm(
