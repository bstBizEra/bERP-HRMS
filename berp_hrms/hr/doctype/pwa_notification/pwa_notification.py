...
class PWANotification(Document):
	...
	def on_update(self):
		berp_hrms.refetch_resource("berp_hrms:notifications", self.to_user)
	...
	def send_push_notification(self):
		try:
			...
			push_notification = PushNotification("berp_hrms")
			if push_notification.is_enabled():
				push_notification.send_notification_to_user(
					...
					icon=f"{frappe.utils.get_url()}/assets/berp_hrms/manifest/favicon-196.png",
				)
		except ImportError:
	...
	def get_notification_link(self):
		base_url = f"{frappe.utils.get_url()}/berp_hrms"
