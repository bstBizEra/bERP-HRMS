...
frappe.ui.form.on("Job Applicant", {
		...
		frm.make_methods = {
			Employee: () =>
				frappe.model.open_mapped_doc({
					method: "berp_hrms.hr.doctype.job_applicant.job_applicant.make_employee",
					frm: frm,
				}),
		};
	},
	...
	get_interview_for_dashboard: function (frm) {
		...
		frappe.call({
			method: "berp_hrms.hr.doctype.job_applicant.job_applicant.get_interview_details",
			args: {
				job_applicant: frm.doc.name,
			},
			callback: function (r) {
	...
	show_schedule_dialog: function (frm) {
		let d = new frappe.ui.Dialog({
			...
			primary_action(values) {
				frappe.call({
					method: "berp_hrms.hr.doctype.job_applicant.job_applicant.schedule_interview",
					args: {
						job_applicant: frm.doc.name,
