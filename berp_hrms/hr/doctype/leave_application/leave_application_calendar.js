...
function refresh_leave_metrics(quick_entry) {
	...
	frappe
		.call({
			method: "berp_hrms.hr.doctype.leave_application.leave_application.get_leave_metrics_and_details",
			args: {
				employee,
				leave_type,
				from_date,
...
function sync_leave_approver(quick_entry, employee, options = {}) {
	...
	return frappe
		.call({
			method: "berp_hrms.hr.doctype.leave_application.leave_application.get_leave_approver_and_mandatory",
			args: {
				employee,
			},
		})
...
frappe.views.calendar["Leave Application"] = {
	...
	options: {
		...
		select: async function (info) {
			...
			doc.to_date = to_date;
			doc.employee =
				(await berp_hrms.get_current_employee()) ||
	...
	get_events_method: "berp_hrms.hr.doctype.leave_application.leave_application.get_events",
};
