frappe.listview_settings["Employee Checkin"] = {
	...
	onload: function (listview) {
		listview.page.add_action_item(__("Fetch Shifts"), () => {
			...
			frappe.call({
				method: "berp_hrms.hr.doctype.employee_checkin.employee_checkin.bulk_fetch_shift",
				freeze: true,
				args: {
					checkins,
				},
...
async function setup_checkin_action(listview) {
	const employee = await frappe.xcall("berp_hrms.api.get_current_employee_info");
	if (!employee) return;
...
async function start_checkin(listview, next) {
	...
	try {
		coordinates = await berp_hrms.get_current_position();
	...
	if (failure) {
		return frappe.msgprint({
			message: berp_hrms.get_geolocation_error_message(failure),
