...
frappe.ui.form.on("Leave Adjustment", {
	refresh(frm) {
		...
		frm.set_query("leave_type", () => {
			return {
				query: "berp_hrms.hr.doctype.leave_adjustment.leave_adjustment.get_allocated_leave_types",
				filters: {
	...
	set_leave_allocation: function (frm) {
		if (frm.doc.posting_date && frm.doc.employee && frm.doc.leave_type) {
			frappe.call({
				method: "berp_hrms.hr.doctype.leave_adjustment.leave_adjustment.get_leave_allocation_for_posting_date",
				args: {
