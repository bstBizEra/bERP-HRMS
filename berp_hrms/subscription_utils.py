...
def subscription_updated(app: str, plan: str):
	if app in ["berp_hrms", "erpnext"] and plan:
		update_erpnext_access()


def update_erpnext_access(user_input: dict | None):
	"""
	Called from hooks after setup wizard completion, ignored if user has no berp_hrms subscription
	enables erpnext workspaces and roles if user has subscribed to both berp_hrms and erpnext
	disables erpnext workspaces and roles if user has subscribed to berp_hrms but not erpnext
	"""
...
def set_app_logo():
	frappe.db.set_single_value("Navbar Settings", "app_logo", "/assets/berp_hrms/images/frappe-hr-logo.svg")


def get_erpnext_roles() -> set:
	erpnext_roles = get_roles_for_app("erpnext")
	berp_hrms_roles = get_roles_for_app("berp_hrms")
	return erpnext_roles - berp_hrms_roles - set(STANDARD_ROLES)
