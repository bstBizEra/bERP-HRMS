frappe.provide("berp_hrms.telemetry");

// HR workspaces shown on the desk sidebar (see berp_hrms/hr/workspace + payroll/workspace).
...
function hr_capture(event, props) {
	...
	try {
		frappe.telemetry.capture(event, "berp_hrms", props || {});
	} catch (e) {
...
function track_landing() {
	try {
		if (sessionStorage.getItem("berp_hrms_landing_tracked")) return;
		sessionStorage.setItem("berp_hrms_landing_tracked", "1");
	} catch (e) {
