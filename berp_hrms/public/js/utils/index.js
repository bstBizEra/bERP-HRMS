frappe.provide("berp_hrms");

$.extend(berp_hrms, {
	proceed_save_with_reminders_frequency_change: () => {
		frappe.ui.hide_open_dialog();
		frappe.call({
			method: "berp_hrms.hr.doctype.hr_settings.hr_settings.set_proceed_with_frequency_change",
			callback: () => {
	...
	handle_realtime_bulk_action_notification: (frm, event, doctype) => {
		...
		frappe.realtime.on(event, (message) => {
			berp_hrms.notify_bulk_action_status(
				doctype,
				message.failure,
				message.success,
	...
	fetch_geolocation: async (frm) => {
		...
		try {
			const { latitude, longitude } = await berp_hrms.get_current_position();
			...
			frappe.msgprint({
				message: berp_hrms.get_geolocation_error_message(error),
