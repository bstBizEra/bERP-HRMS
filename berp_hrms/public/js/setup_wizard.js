frappe.provide("berp_hrms.setup");

frappe.setup.on("before_load", function () {
	if (
		frappe.boot.setup_wizard_completed_apps?.length &&
		frappe.boot.setup_wizard_completed_apps.includes("berp_hrms")
	) {
		return;
	}
	...
	if (persona_index >= 0) {
		frappe.setup.slides.splice(persona_index, 1, ...berp_hrms.setup.slides_settings);
	} else {
		berp_hrms.setup.slides_settings.map(frappe.setup.add_slide);
	}
});

berp_hrms.setup.slides_settings = [
	{
		...
		apply_company_size_modules: function (slide) {
			...
			let modules = berp_hrms.setup.company_size_modules[company_size] || [
				"leave_attendance",
				"payroll",
			];
...
berp_hrms.setup.company_size_modules = {
