...
"""berp_hrms ships its navigation as `Sidebar` fixtures, one per semantic module.
...
why it is asserted: the failure mode is berp_hrms's curated navigation quietly reverting to generated.
...
#: The semantic modules that own berp_hrms's navigation, as declared in `modules.txt`, each mapped to
