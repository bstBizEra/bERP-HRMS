...
from berp_hrms.telemetry import (
	CONVERSION_EVENT,
...
class TestTelemetry(HRMSTestSuite):
	def setUp(self):
		...
		patches = [
			patch("berp_hrms.telemetry._skip_context", return_value=False),
			patch("berp_hrms.telemetry.is_enabled", return_value=True),
			patch("berp_hrms.telemetry._capture", side_effect=collect),
			patch("berp_hrms.telemetry.site_age", return_value=3),
			patch("berp_hrms.telemetry.today", return_value=FROZEN_TODAY),
		]
		for p in patches:
			p.start()
	...
	def test_nothing_is_captured_or_recorded_when_telemetry_is_disabled(self):
		...
		with patch("berp_hrms.telemetry.is_enabled", return_value=False):
	...
	def test_milestone_skipped_outside_activation_window(self):
		...
		with patch("berp_hrms.telemetry.site_age", return_value=90):
			create_leave_type()
